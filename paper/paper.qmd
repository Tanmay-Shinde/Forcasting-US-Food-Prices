...
In order to predict price trends, we use a Bayesian Hierarchical Model (BHM) with ARIMA components to address the hierarchical structure of the data as well as to account for time-series dynamics.

## Model set-up

Let $y_{ijt}$ denote the log-transformed unit value (unitValue_lg) for region $i$, category $j$ and time $t$. Then, the model can be written as:

$$y_{ijt} = \beta_0 + \beta^{region}_i + \beta^{category}_j + \beta_1.CPI_{ijt} + \beta_2.purchaseVolume_{ijt} + \phi.y_{ij(t-1)} + \epsilon_{ijt}$$

Where:

- $\beta_0$: Global intercept (average log unit value across all regions and categories).

- $\beta^{region}_i$: Random effect for region $i$, capturing region-specific deviations.

- $\beta^{category}_j$: Random effect for category $j$, capturing category-specific deviations.

- $\beta_1$: Coefficient for the CPI predictor.

- $\beta_2$: Coefficient for the purchaseVolume predictor.

- $\phi$: AR(1) term capturing temporal autocorrelation. More information about the AR(1) model can be found in [Appendix -@sec-model-details].
...
- $\beta^{region}_i \sim N(0, \sigma^2_{region})$: Region-level random effects.

- $\beta^{category}_j \sim N(0, \sigma^2_{category})$: Category-level random effects.

- $\beta_1, \beta_2 \sim N(0, 10^2)$: Weakly informative priors for the predictors

- $\phi \sim N(0,1)$: Prior for the AR(1) term, constrained to [-1, 1] to ensure stationarity.

- $\sigma^2, \sigma^2_{region}, \sigma^2_{category} \sim HalfCauchy(0,5)$: Priors for variance components.
...
@fig-timetrend-cpi also illustrates the general upward trend in the CPI values from 2012 to 2018.
...
#| warning: false
#| message: false
#| label: fig-bxplt-pvol
#| fig-cap: "Regional Variations in Purchase Volume"
...
```
From @fig-bxplt-pvol, we can see that across all regions, the mean purchase volume is relatively consistent, suggesting similar central tendencies in the amount of food purchased. However, the South exhibits a slightly higher mean and a wider interquartile range, indicating greater variability in purchase volume compared to other regions - which might be due to the lower cpi values in the South as revealed in @fig-bxplt-cpi above.
...
- $\phi$: The AR(1) coefficient (a scalar between -1 and 1) that captures the strength and direction of temporal autocorrelation.

- $y_{t-1}$: Value of the variable at time $t-1$ (previous time step).

- $\epsilon_t$: Random error or noise term at time $t$ assumed to be independently and identically distributed ($\epsilon_{ijt} \sim N(0, \sigma^2)$).
...
Role of the $\phi$ Term: 

The $\phi$ term captures temporal autocorrelation, which is the relationship between observations at consecutive time steps. Here's what it represents:
